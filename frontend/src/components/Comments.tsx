...
type CommentsProps = {
    open: boolean;
    postId: PostId;
    onCommentsUpdated: (params: CommentsUpdatedParams) => void;
}
...
export function Comments({ open, postId, onCommentsUpdated }: CommentsProps) {
    ...
    const getData = async ({ page_num }: GetDataParams = { page_num: 1 }) => {
        const result = await apiGet<Paginated<CommentDTO>>(`/comments`, { page_num, post_id: postId });
    ...
    const loadMoreOnClick = async () => {
        ...
        const result = await apiGet<Paginated<CommentDTO>>(`/comments`, { page_num: pageNum + 1, post_id: postId });
        if (result.ok && result.data) {
            const { page_items, page_size, page_num, total } = result.data;
    ...
    const handleCommentDeleted = (comment: CommentDTO) => {
